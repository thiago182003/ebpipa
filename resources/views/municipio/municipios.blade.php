...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-0"><span class="text-muted fw-light">    Cadastro /</span> Municipios
        </h4>
        <div class="row">
            <div class="col-md-12">
            ...
            <div class="col-md-12">
                <div class="card mb-3">
                                        ...
                                        <tbody>
                                            @foreach ($municipios as $mun)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{ @$mun->nome }}</td>
                                                    <td>{{ @$mun->estado->sigla }}</td>
                                                    <td>{{ @$mun->om->sigla }}</td>
                                                    <td>
                                                        <a href="{{ route('municipio.editar', @$mun->id) }}"
                                                            class="btn btn-sm btn-icon btn-outline-primary">
                                                            <span class="tf-icons bx bx-edit"></span>
                                                        </a>
                                                    </td>
                                                    <td>
                                                        <button data-id={{@$mun->id}} data-nome="{{@$mun->nome}}" class="btn btn-sm btn-icon btn-outline-danger remover">
                                                            <i class='bx bx-x'></i>
                                                        </button>
                                                    </td>
                                                </tr>
                    ...
                    <div class="mt-2 save-now">
                        <a href="{{ route('municipio.novo')}}" type="submit" class="btn btn-success btn-novo-now"><i class='bx bx-plus'></i>
                            Novo</a>
                    </div>
                </div>
            </div>
                ...
                $('#'+nomeTabela+' tbody').find('tr').each(function (indexTr, valueTr) {
                    $(this).find('td').each(function (indexTd, valueTd) {
                        if (indexTd === 0) {
                            ...
                            // if ($(valueTd).html().indexOf(pesquisa) === -1) {
                                $(valueTd).parent().hide();
                            } else {
                                $(valueTd).parent().show();
                            }
